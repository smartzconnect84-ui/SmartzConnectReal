...
import {
  ...
  Radio, ShoppingBag, Car, Star, Infinity, Smartphone,
...
/* ─────────────── Canonical plan definitions ─────────────────────────────────
   IDs must match DB: free | premium | vip
   The page falls back to these if subscription_plans table is empty.
───────────────────────────────────────────────────────────────────────────── */
type Feature = { text: string; included: boolean; highlight?: boolean }
type Plan = {
  id: string; name: string; priceUSD: number; icon: typeof Heart
  color: string; border: string; glowClass: string
  badge: string | null; period: string; tagline: string; emoji: string
  features: Feature[]; cta: string; ctaStyle: string
}

const PLANS: Plan[] = [
  {
    id: 'free', name: 'Free', priceUSD: 0, icon: Heart,
    color: 'from-slate-500 to-gray-600',
    border: 'dark:border-white/10 border-gray-200',
    glowClass: '',
    badge: null, period: 'forever',
    tagline: 'Get the full SmartzConnect ecosystem — forever free to start',
    emoji: '🆓',
    features: [
    ...
    ctaStyle: 'dark:bg-white/8 bg-gray-100 dark:text-white text-gray-900 hover:dark:bg-white/14 hover:bg-gray-200',
  },
  {
    id: 'premium', name: 'Premium', priceUSD: 5, icon: Zap,
    color: 'from-pink-500 to-rose-600',
    border: 'border-pink-500/50',
    glowClass: 'shadow-2xl shadow-pink-500/20',
    badge: 'MOST POPULAR', period: 'per month',
    tagline: 'For active creators, daters and shoppers who want more',
    emoji: '⚡',
    features: [
  ...
  {
    id: 'vip', name: 'VIP', priceUSD: 10, icon: Crown,
    color: 'from-amber-500 to-yellow-500',
    border: 'border-amber-500/50',
    glowClass: 'shadow-2xl shadow-amber-500/20',
    badge: 'FULLY UNLIMITED', period: 'per month',
    tagline: 'For vendors, drivers, broadcasters and businesses',
    emoji: '👑',
    features: [
    ...
    cta: 'Go VIP →',
    ctaStyle: 'bg-gradient-to-r from-amber-500 to-yellow-500 text-white shadow-lg shadow-amber-500/30 hover:shadow-amber-500/50 hover:scale-[1.02]',
  },
]

/* ─────────────── Feature comparison table ───────────────────────────────────
   Column keys: free | premium | vip  — match plan IDs exactly
───────────────────────────────────────────────────────────────────────────── */
const featureRows: {
  icon: typeof Heart; label: string
  free: boolean | string | React.ReactNode
  premium: boolean | string | React.ReactNode
  vip: boolean | string | React.ReactNode
}[] = [
  ...
  { icon: Star,          label: 'Discover Priority',        free: false,     premium: '3×',       vip: '🔝 Top 5×'  },
  { icon: Play,          label: 'SmartzTV Streaming',       free: 'Watch',   premium: true,       vip: true         },
  { icon: Radio,         label: 'Go Live',                  free: false,     premium: true,       vip: true         },
  { icon: ShoppingBag,   label: 'Marketplace Seller',       free: false,     premium: true,       vip: true         },
  { icon: MessageCircle, label: 'Group Chat Rooms',         free: 'Join',    premium: true,       vip: true         },
  ...
  { icon: MapPin,        label: 'WorldStage Access',        free: false,     premium: false,      vip: true         },
]
...
const faqs = [
  {
    ...
    a: "No credit card needed! We accept MTN Mobile Money and Orange Money. Send the amount to our number, enter your Transaction ID in the app, and your plan activates within 15 minutes.",
  },
  {
    q: 'Can I cancel or downgrade anytime?',
    a: 'Yes, absolutely. No long-term contracts. Cancel from App → Subscriptions anytime. Your access continues until the end of the current billing period.',
  },
  {
    q: "What's the price in my local currency?",
    a: 'Use our currency converter on this page to see prices in 25+ African and global currencies. Payments are accepted as USD-equivalent via Mobile Money.',
  },
  {
    q: 'Is the Free plan really free forever?',
    a: 'Yes! Free Forever means no credit card, no trial expiry — you keep your 10 daily swipes, social feed, marketplace browsing, ride booking, and community rooms forever at zero cost.',
  },
  {
    q: 'How is Premium different from VIP?',
    a: 'Premium ($5) unlocks unlimited swipes, Go Live, voice & video calls, stories, marketplace selling, and ad-free access across all 8 products. VIP ($10) adds everything in Premium — plus your Verified badge, top-of-Discover placement (5× boost), Super Likes, creator revenue share, advanced analytics, WorldStage access, and dedicated 24/7 VIP support.',
  },
  {
    q: 'How long does payment verification take?',
    a: 'Our admin team verifies Mobile Money payments within 15 minutes during business hours (8 AM–10 PM WAT). After hours it may take up to 2 hours. You receive an in-app notification the moment your plan activates.',
  },
  {
    q: 'Does the yearly discount apply automatically?',
    a: 'Yes. Switch the billing toggle to "Yearly" and prices drop by 20% — Premium goes from $5 to $4/month, VIP from $10 to $8/month. You are billed once for the full year upfront.',
  },
]

/* ─────────────── Payment methods ──────────────────────────────────────────── */
const payMethods = [
  { name: 'MTN MoMo',     emoji: '📱', detail: '+231 888 061 379', color: 'bg-yellow-500/10 border-yellow-500/25 text-yellow-700 dark:text-yellow-400', active: true  },
  { name: 'Orange Money', emoji: '🟠', detail: '+231 776 679 963', color: 'bg-orange-500/10 border-orange-500/25 text-orange-700 dark:text-orange-400', active: true  },
  { name: 'Stripe / Card',emoji: '💳', detail: 'Coming soon',      color: 'bg-blue-500/10 border-blue-500/25 text-blue-700 dark:text-blue-400',          active: false },
  { name: 'PayPal',       emoji: '🅿️', detail: 'Coming soon',      color: 'bg-indigo-500/10 border-indigo-500/25 text-indigo-700 dark:text-indigo-400',  active: false },
]
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2 pt-1">
          ...
          {PLANS.filter(p => p.priceUSD > 0).map(p => (
            <div key={p.id} className="flex items-center justify-between py-2 px-3 rounded-xl dark:bg-white/4 bg-gray-50 border dark:border-white/6 border-gray-100">
...
export default function PricingPage() {
  ...
  // Try to load live plan data from DB; fall back to PLANS if unavailable
  useEffect(() => {
    let mounted = true
    supabase
      ...
      .then(({ data, error }) => {
        ...
        if (!error && data && data.length >= 2) {
          const validRows = data.filter((row: any) =>
            PLANS.some(p => p.id === (row.id ?? '').toLowerCase())
          )
          if (validRows.length < 2) return // keep hardcoded PLANS
          const mapped: Plan[] = validRows.map((row: any) => {
            const base = PLANS.find(p => p.id === (row.id ?? '').toLowerCase())!
        ...
        // on error or < 2 rows keep hardcoded PLANS
      })
    return () => { mounted = false }
  }, [])
    ...
    <div className="dark:bg-[#080510] bg-white min-h-screen">
      ...
      <section className="pb-16 sm:pb-20" ref={plansRef}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
            ...
            {activePlans.map((plan, i) => {
              ...
              return (
                <motion.div key={plan.id}
                  ...
                  className={`relative dark:bg-[#130E1E] bg-white rounded-3xl p-6 lg:p-7 border-2 ${plan.border} flex flex-col ${plan.glowClass} ${plan.id === 'premium' ? 'md:-mt-5 md:mb-5' : ''} hover:scale-[1.01] transition-all duration-300`}
                >
                  {plan.badge && (
                  ...
                  <ul className="space-y-2.5 mb-7 flex-1">
                    {plan.features.map(f => (
                      <li key={f.text} className={`flex items-start gap-2.5 text-sm ${
                        f.included
                          ? f.highlight ? 'dark:text-white text-gray-900 font-medium' : 'dark:text-gray-300 text-gray-700'
                          : 'dark:text-gray-600 text-gray-400 line-through decoration-gray-300 dark:decoration-gray-700'
                      }`}>
                        {f.included
          ...
          <motion.div
            initial={{ opacity: 0, y: 30 }} animate={plansIn ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.4 }}
          ...
          >
            ...
            <div className="grid grid-cols-4 border-b dark:border-white/6 border-gray-100 bg-gradient-to-r dark:from-white/2 from-gray-50 to-transparent">
              <div className="p-5">
                <p className="font-bold dark:text-gray-300 text-gray-700 text-sm">Feature comparison</p>
              </div>
              {activePlans.map(p => (
                <div key={p.id} className="p-5 text-center border-l dark:border-white/5 border-gray-100">
            ...
            {featureRows.map((row, i) => {
              ...
              return (
                <div key={row.label} className={`grid grid-cols-4 ${i < featureRows.length - 1 ? 'border-b dark:border-white/4 border-gray-100' : ''}`}>
                  ...
                  {([row.free, row.premium, row.vip] as const).map((val, j) => (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-5">
              ...
              <div className="dark:bg-[#130E1E] bg-white rounded-2xl border dark:border-white/8 border-gray-200 p-5">
                ...
                {[
                  { step: '1', text: 'Pick a plan and tap "Start Premium" or "Go VIP"' },
                  { step: '2', text: 'Send the USD-equivalent amount to our MTN or Orange Money number' },
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          ...
          <div className="overflow-hidden rounded-2xl border dark:border-white/8 border-gray-200 dark:bg-[#130E1E] bg-white shadow-xl">
            <div className="grid grid-cols-4 bg-gradient-to-r dark:from-white/3 from-gray-50 to-transparent border-b dark:border-white/6 border-gray-100">
              ...
              {activePlans.map(p => (
                <div key={p.id} className="p-4 text-center text-xs font-bold dark:text-gray-400 text-gray-500 uppercase tracking-wider">
                  {p.emoji} {p.name}
                </div>
              ))}
            </div>
            {CURRENCIES.filter(c => c.popular).map((cur, i, arr) => (
              <div key={cur.code} className={`grid grid-cols-4 ${i < arr.length - 1 ? 'border-b dark:border-white/4 border-gray-100' : ''} hover:dark:bg-white/2 hover:bg-gray-50 transition-colors`}>
                ...
                {activePlans.map(p => (
                  <div key={p.id} className="p-4 text-center flex items-center justify-center">
            ...
            <div className="p-4 text-center border-t dark:border-white/6 border-gray-100">
              <button onClick={() => setShowAll(s => !s)}
                ...
                {showAll
                  ...
                  : <><ChevronDown className="w-3.5 h-3.5" /> Show all {CURRENCIES.length} currencies</>
                }
              </button>
            </div>
            <AnimatePresence>
              {showAll && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                  {CURRENCIES.filter(c => !c.popular).map(cur => (
      ...
      <section className="pb-20 dark:bg-[#080510] bg-gray-50" ref={faqRef}>
        <div className="max-w-2xl mx-auto px-4 sm:px-6">
          ...
          <div className="space-y-3">
            {faqs.map((faq, i) => (
              <motion.div key={i}
                initial={{ opacity: 0, y: 20 }} animate={faqIn ? { opacity: 1, y: 0 } : {}} transition={{ delay: i * 0.06 }}
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          <div className="relative overflow-hidden rounded-3xl bg-love-gradient p-10 text-center shadow-2xl shadow-pink-500/25">
            ...
            <p className="text-white/80 text-base mb-7 max-w-lg mx-auto">
              Join our growing community across Africa — build real connections, businesses, and communities on SmartzConnect.
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link to="/register"
            ...
            <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-6 mt-6">
              {['No credit card', 'Cancel anytime', '15-min activation', '8 products included'].map(t => (
