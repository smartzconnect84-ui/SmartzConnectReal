...
import { Image, Video, Send, Heart, MessageCircle, Share2, MoreHorizontal, Bookmark, TrendingUp, RefreshCw, MapPin } from 'lucide-react'
...
export default function FeedPage() {
  ...
  const fetchPosts = async () => {
    ...
    // Step 1: fetch posts (no FK join — posts table has no FK to profiles in the schema)
    ...
    // Step 2: fetch profiles for those authors
    const authorIds = [...new Set(postRows.map(p => p.author_id).filter(Boolean))]
    const { data: profiles } = authorIds.length
      ? await supabase
          .from('profiles')
          .select('id, full_name, avatar_url, is_verified, subscription_tier, username')
          .in('id', authorIds)
      : { data: [] }
      ...
      <div className="max-w-xl mx-auto px-3 py-4">
          ...
          <AnimatePresence mode="wait">
            {!showCompose ? (
              <motion.button key="trigger" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                onClick={() => setShowCompose(true)} className="w-full flex items-center gap-3 text-left">
              ...
              <motion.div key="compose" initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-3">
                ...
                <div className="flex items-center justify-between pt-2 border-t dark:border-white/6 border-gray-100">
                  <div className="flex gap-2">
                    <button className="p-2 rounded-xl dark:hover:bg-white/5 hover:bg-gray-100 transition-colors"><Image className="w-4 h-4 dark:text-gray-400 text-gray-500" /></button>
                    <button className="p-2 rounded-xl dark:hover:bg-white/5 hover:bg-gray-100 transition-colors"><Video className="w-4 h-4 dark:text-gray-400 text-gray-500" /></button>
                    <button className="p-2 rounded-xl dark:hover:bg-white/5 hover:bg-gray-100 transition-colors"><MapPin className="w-4 h-4 dark:text-gray-400 text-gray-500" /></button>
                  </div>
                  <div className="flex gap-2">
        ...
        {!loading && error && (
          <div className="flex flex-col items-center justify-center py-16 gap-4 text-center">
            ...
            <p className="text-sm dark:text-gray-400 text-gray-500 max-w-xs">{error}</p>
            <button onClick={fetchPosts}
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-love-gradient text-white text-sm font-bold">
            ...
            <div className="flex items-center gap-3 px-4 py-3.5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  {post.location && (<><span className="dark:text-gray-700 text-gray-300">·</span>
            ...
            <div className="flex items-center justify-between px-4 py-2 border-t dark:border-white/4 border-gray-50">
              <div className="flex gap-1">
                ...
                <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl dark:text-gray-400 text-gray-500 hover:text-brand-pink dark:hover:bg-white/5 hover:bg-gray-50 transition-colors text-xs font-semibold">
                  <MessageCircle className="w-4 h-4" />{post.comments > 0 && post.comments}
                </button>
                <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl dark:text-gray-400 text-gray-500 hover:text-brand-pink dark:hover:bg-white/5 hover:bg-gray-50 transition-colors text-xs font-semibold">
                  <Share2 className="w-4 h-4" />{post.shares > 0 && post.shares}
                </button>
              </div>
        ...
        {/* Trending */}
        {!loading && (
          <div className="dark:bg-[#0D0A14] bg-white rounded-2xl border dark:border-white/6 border-gray-200 p-4 mt-2">
            ...
            <div className="flex flex-wrap gap-2">
              {trending.map(tag => (
                <button key={tag} className="px-2.5 py-1.5 rounded-xl dark:bg-white/5 bg-gray-50 dark:text-pink-300 text-gray-600 text-xs font-semibold hover:text-brand-pink transition-colors">
                  {tag}
                </button>
              ))}
            </div>
