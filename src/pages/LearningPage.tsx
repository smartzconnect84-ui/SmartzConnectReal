...
import { BookOpen, Plus, Loader2, Link2, RefreshCw, Database, Search, ExternalLink, Bookmark, Clock, Eye } from 'lucide-react'
...
const emptyForm: NewResource = { title: '', description: '', category: 'General', resource_type: 'Article', resource_url: '', cover_url: '', duration_mins: '' }
...
export default function LearningPage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const url = await uploadToSufy(file, 'learning')
  ...
  const toggleSave = async (r: ResourceRow) => {
    if (!user) return
    if (r.saved) {
      await supabase.from('learning_saves').delete().eq('resource_id', r.id).eq('user_id', user.id)
    } else {
      await supabase.from('learning_saves').insert({ resource_id: r.id, user_id: user.id })
    }
    fetchResources()
  }
      ...
      <div className="px-4 pt-4 pb-3 flex-shrink-0 dark:bg-[#0D0A14] bg-white border-b dark:border-white/6 border-gray-100">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-xs dark:text-gray-400 text-gray-500">{dbConnected ? `${filtered.length} resources` : 'Connect database'}</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={fetchResources} className="w-9 h-9 rounded-xl dark:bg-white/5 bg-gray-100 flex items-center justify-center hover:text-brand-pink transition-colors">
