...
import { Trophy, Plus, Trash2, Pencil, Check, X, Star, Users, Award } from 'lucide-react'
...
interface WSSpotlight {
  ...
  followers_label: string | null; quote: string | null; avatar_emoji: string; wins: number
...
const emptySpotlight: Omit<WSSpotlight, 'id'> = {
  ...
  avatar_emoji: '⭐', wins: 0, is_active: true, sort_order: 0,
}
          ...
          {editingSpotlight && (
            <div className="mb-6 p-5 rounded-2xl dark:bg-[#130E1E] bg-white border dark:border-white/8 border-gray-200 space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <input placeholder="Avatar emoji" value={editingSpotlight.avatar_emoji} onChange={e => setEditingSpotlight({ ...editingSpotlight, avatar_emoji: e.target.value })} className="input-field" />
            ...
            {spotlights.map(s => (
              <div key={s.id} className="p-4 rounded-2xl dark:bg-[#130E1E] bg-white border dark:border-white/8 border-gray-200">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <div>
                    <p className="font-bold text-sm dark:text-white text-gray-900">{s.display_name}</p>
                    <p className="text-xs dark:text-gray-400 text-gray-500">{s.category} · {s.country}</p>
                  </div>
                </div>
                <p className="text-xs italic dark:text-gray-400 text-gray-500 mb-3 line-clamp-2">"{s.quote}"</p>
                <div className="flex items-center gap-2">
                  <button onClick={() => toggleSpotlightActive(s)} className={`flex-1 px-3 py-1.5 rounded-lg text-xs font-bold ${s.is_active ? 'bg-emerald-500/10 text-emerald-500' : 'bg-gray-500/10 text-gray-400'}`}>
